  ...
  return <div className="min-h-screen flex flex-col relative">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center py-4 px-6 bg-black/50 backdrop-blur-sm">
        <div className="flex gap-4 md:gap-6">
          {Object.keys(sectionsRef.current).map(section => <button key={section} onClick={() => scrollToSection(section)} className={`relative px-3 py-1 text-sm transition-all duration-300 ${activeSection === section ? 'text-primary' : 'text-gray-400 hover:text-gray-200'}`}>
              <span>{section.charAt(0).toUpperCase() + section.slice(1)}</span>
              {activeSection === section && <motion.div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary" layoutId="activeSection" transition={{
            type: "spring",
            duration: 0.5
          }} />}
            </button>)}
      ...
      <section ref={el => sectionsRef.current.hero = el} className="min-h-screen flex items-center justify-center hero-gradient-bg overflow-hidden">
        ...
        <motion.div className="purple-blob absolute w-96 h-96 top-20 left-20 opacity-30 z-0" initial={{
        scale: 0.8,
        opacity: 0.2
      }} animate={{
        scale: [0.8, 1.2, 0.8],
        ...
        <motion.div className="purple-blob absolute w-96 h-96 bottom-20 right-20 opacity-30 z-0" initial={{
        scale: 1,
        opacity: 0.3
      }} animate={{
        scale: [1, 0.7, 1],
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-0 opacity-10">
          <motion.div className="sound-wave flex items-center justify-center gap-1" initial="hidden" animate="visible">
            {[...Array(12)].map((_, i) => <motion.span key={i} className={`w-1 rounded-full ${i % 2 === 0 ? 'bg-primary' : 'bg-[#BD00FF]'}`} initial={{
            height: 5
          }} animate={{
            height: [5, 30, 5]
          }} transition={{
        ...
        <motion.div className="container mx-auto px-4 text-center z-10" initial="hidden" animate="visible" variants={containerVariants}>
          {/* Centered Logo */}
          ...
          <motion.p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-2xl mx-auto" variants={itemVariants}>
          ...
          <div className="w-full flex justify-center">
            <motion.button className="scroll-down-btn mt-20" onClick={() => scrollToSection('features')} variants={itemVariants} whileHover={{
            y: 5
          }}>
      ...
      <section ref={el => sectionsRef.current.features = el} className="py-20 bg-black">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map((feature, index) => <motion.div key={feature.title} className="feature-card p-6 rounded-xl" initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            opacity: 1,
          ...
          <div className="flex justify-center mt-16">
            <motion.button className="scroll-down-btn" onClick={() => scrollToSection('getStarted')} whileHover={{
            y: 5
          }}>
              <ChevronDown className="w-8 h-8 text-primary animate-bounce" />
            </motion.button>
      ...
      <section ref={el => sectionsRef.current.getStarted = el} className="py-20 bg-gradient-to-b from-black to-card/80">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center mt-16">
            <motion.button className="scroll-down-btn" onClick={() => scrollToSection('demo')} whileHover={{
            y: 5
          }}>
              <ChevronDown className="w-8 h-8 text-primary animate-bounce" />
            </motion.button>
      ...
      <section ref={el => sectionsRef.current.demo = el} className="py-20 bg-gradient-to-b from-card/80 to-black">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center mt-16">
            <motion.button className="scroll-down-btn" onClick={() => scrollToSection('cta')} whileHover={{
            y: 5
          }}>
              <ChevronDown className="w-8 h-8 text-primary animate-bounce" />
            </motion.button>
      ...
      <section ref={el => sectionsRef.current.cta = el} className="py-20 bg-gradient-to-br from-black to-card">
        <div className="container mx-auto px-4 text-center">
          <motion.div className="mb-8 inline-flex" initial={{
          y: 0
        }} animate={{
          y: [-10, 10, -10]
        }} transition={{
          ...
          once: true
            ...
            <div className="flex justify-center mt-16">
              <motion.button className="scroll-down-btn" onClick={() => scrollToSection('hero')} whileHover={{
              y: -5
            }}>
